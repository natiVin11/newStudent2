...
app.post("/api/upload", upload.single("file"), (req, res) => {
    ...
    db.run("UPDATE students SETfile_path = ? WHERE id_number = ?", [relativeFilePath, idNumber], function (err) {
        if (err) {
...
// --- API Endpoint for fetching student progress ---
app.get("/api/students/progress", (req, res) => {
    console.log("GET /api/students/progress - Request received");
    db.all(`SELECT id, name, step FROM students ORDER BY name`, [], (err, rows) => {
        if (err) {
            console.error("API Error GET /api/students/progress:", err.message);
            return res.status(500).json({ error: "Database error fetching student progress" });
        }
        console.log("GET /api/students/progress - Fetched", rows.length, "student progress records");
        res.json(rows);
    });
});
